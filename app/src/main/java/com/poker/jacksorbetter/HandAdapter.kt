...
class HandAdapter(
    ...
    private var handsShown = MutableList(itemCount) {false}
    private val LITTLE_CARD_THRESHOLD = 9
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val timeInMillis = measureTimeMillis {
            ...
            when(state){
                ...
                State.CARD_BACK -> {
                    showBigOrSmallCardBack(cardFronts)
                    holder.handEvalLayout.visibility = View.INVISIBLE
                    hold = mutableListOf()
                    handsShown = MutableList(evals.size) {false}
                }

                State.HOLD -> {
                    item.forEachIndexed { index, it ->
                        if (it in hold) {
                            showBigOrSmallCards(cardFronts[index], it)
                        } else {
                            showBigOrSmallCardBack(cardFronts[index])
                        }
                    }
                }

                State.FLIP -> {
                    showBigOrSmallCardBack(cardFronts) //todo why is this here
                    item.forEachIndexed { index, it ->
                        showBigOrSmallCards(cardFronts[index], it)
                    }

                    if(position < handsShown.size){
                        handsShown[position] = true
                    }
...
//    fun isAllHandsShown(): Boolean {
//        return handsShown.filter { !it }.count() == 0
//    }
...
//        this.evals = evals
        Timber.d("eval size %d", this.evals.size)
        notifyDataSetChanged()
    }
